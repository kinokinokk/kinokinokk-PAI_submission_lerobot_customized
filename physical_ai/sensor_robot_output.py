# PhysicalAI最終課題メインコード
# sensor--robot--output

import subprocess
import time

#以下、predict_dice_classifier.pyのライブラリimport部分
import cv2
import torch
...
#commandの定義部 ーーーーーーーーーーーーーーーーーーーーーーーー
...
command1 = [
    ...
    "--robot.id=purple",
    "--dataset.repo_id=kinokinokk/eval_saikoro1_58",
    "--dataset.num_episodes=1",
    "--dataset.single_task=pick_saikoro",
    "--dataset.episode_time_s=17",
    "--dataset.reset_time_s=3",
    "--policy.path=../outputs/train/pick_saikoro1/checkpoints/last/pretrained_model",
    "--display_data=true"
]
